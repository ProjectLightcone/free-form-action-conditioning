...
from tepa_eval.engine import (
    ...
    should_stop_early,
    target_sigreg_weight_for_epoch,
    write_run_report,
    write_target_sample_panel,
)
from tepa_eval.generate import generate_dataset
from tepa_eval.io import write_jsonl
from tepa_eval.metrics import average_metrics, target_autoencoder_loss_and_metrics
...
def pretrain_target(config_path: str | Path, device_name: str = "auto") -> Path:
    ...
    model = build_from_config(TARGET_MODEL_NAME, config).to(device)
    optimizer = make_optimizer(model, config)
    train_loader = make_target_loader(config, "train", shuffle=True)
    val_loader = make_target_loader(config, "val", shuffle=False)

    run_dir = config.run_dir / f"{int(time.time())}_{TARGET_MODEL_NAME}"
    ...
    for epoch in range(config.epochs):
        ...
        sigreg_weight = target_sigreg_weight_for_epoch(config, epoch)
        model.train()
        train_rows = []
        for batch in tqdm(train_loader, desc=f"{TARGET_MODEL_NAME} epoch {epoch + 1}/{config.epochs}"):
            batch = batch_to_device(batch, device)
            optimizer.zero_grad(set_to_none=True)
            predictions = model_forward(model, batch)
            loss, metrics = target_autoencoder_loss_and_metrics(
                predictions,
                batch,
                config.horizon,
        ...
        val_metrics = evaluate_target_loader(model, val_loader, config, device, sigreg_weight=sigreg_weight)
        row = {
            "epoch": float(epoch + 1),
            "learning_rate": float(learning_rate),
            "target_sigreg_weight": float(sigreg_weight),
            **{f"train_{key}": value for key, value in train_metrics.items()},
            **{f"val_{key}": value for key, value in val_metrics.items()},
        }
        monitor_name = _resolve_monitor(config.early_stopping_monitor, row, ("val_target_reconstruction_loss", "val_loss"))
        ...
        epoch_checkpoint = _target_checkpoint(
            model=model,
            config=config,
            epoch=epoch + 1,
            history=history,
    ...
    final_metrics: dict[str, dict[str, float] | float] = {
        "train": evaluate_target_loader(model, train_loader, config, device),
        "val": evaluate_target_loader(model, val_loader, config, device),
        "train_unique_outcomes": float(len(train_loader.dataset)),
        "val_unique_outcomes": float(len(val_loader.dataset)),
    ...
    write_run_report(run_dir, TARGET_MODEL_NAME, config, history, final_metrics, training_summary)
    write_target_sample_panel(run_dir, model, val_loader, device, config)
    print(f"Saved target pretraining run to {run_dir} (best epoch {best_epoch})")
    return run_dir
...
def _target_checkpoint(
    *,
    ...
    checkpoint: dict[str, Any] = {
        ...
        "target_decoder_state": model.target_decoder.state_dict(),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Pretrain the puck-world target encoder and decoder.")
    ...
    pretrain_target(args.config, device_name=args.device)
